...
- [x] Git worktree create / adopt / delete with visible progress and failure
      recovery
- [x] Workspace status (branch, diffstat, attention state), so the rail's status
      dots *(landed in M2)*
...
"artemis: ready artemis 4", which is the status dot's label, the name, and the change
...
- Assistant footer: copy, fork, finished-at timestamp. No thumbs up/down, because the
...
**Exit:** a transcript that reads like Codex's. A prompt with its metadata, a turn
...
button, which advertises an interaction and then refuses it.
...
The path is vetted through the same `paths::vetted` the undo uses, and it was
...
- **Path traversal.** `/etc/passwd` is not absolute on Windows, having no drive
  letter, so it passed vetting, and `join` there *replaces* a rooted path rather than
...
## M14. Open-source readiness: public, CI green, one setting left
...
- ✅ Repo public at https://github.com/c-wenlong/artemis
